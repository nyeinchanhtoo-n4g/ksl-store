...
export const salesStatementSchema = z.object({ date: z.string().min(1), waybillNo: z.string().trim().max(100).optional().or(z.literal("")), receiverName: z.string().trim().max(120).optional().or(z.literal("")), productName: z.string().trim().min(1).max(160), toCity: z.string().trim().max(120).optional().or(z.literal("")), price: money, prepayment: money, deliCharge: money, codCharge: money, codAmount: money, closingBalance: money, note: z.string().trim().max(1_000).optional().or(z.literal("")) });
export const expenseStatementSchema = z.object({ date: z.string().min(1), productName: z.string().trim().min(1).max(160), cost: money, quantity: z.union([z.literal(""), wholeNumberInput().pipe(z.number().nonnegative())]).transform((value) => value === "" ? null : value), totalCost: money, deliCharge: money, closingAmount: money, note: z.string().trim().max(1_000).optional().or(z.literal("")) });
